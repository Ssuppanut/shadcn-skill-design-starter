...
4. Kill layout shift: size skeletons to final dimensions, reserve media space via `aspect-ratio` (`_ux-ui-shared/tokens/sizing.json`), never inject content above existing.
5. Animation: only `transform`/`opacity`, `will-change` sparingly, 100–300ms (`_ux-ui-shared/tokens/motion.json`), honor `prefers-reduced-motion`. Prefer CSS state styling over JS for low INP; tree-shake/per-component imports.
