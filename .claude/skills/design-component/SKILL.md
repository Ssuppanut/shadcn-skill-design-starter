...
## Steps
1. Read `_ux-ui-shared/CLAUDE.md` → "Component Guidelines" (quality bar, the 8-state table) and "Atomic Design".
2. Check if it already exists: `_ux-ui-shared/components/atoms.md`, `molecules.md`, `organisms.md`, `templates.md`, `navigation.md`, `feedback.md`, `forms-advanced.md`, `overlays.md`. Match the existing spec format.
3. Pull the ARIA pattern from `_ux-ui-shared/accessibility/aria-patterns.md` and contrast/target rules from `_ux-ui-shared/accessibility/wcag-checklist.md`.
4. Map every value to tokens (`_ux-ui-shared/tokens/*.json`) — sizes via `sizing.json`, states via `states.json`.
5. Apply visual judgment from `_ux-ui-shared/taste/design-taste.md` (states, focus, no slop).
6. Optional fast start: `python3 _ux-ui-shared/scripts/scaffold_component.py "<Name>"` to emit a stub, then fill it in.

## Output
Spec with: anatomy diagram, variants table, sizes table, all 8 applicable states, token mapping, accessibility (role/keyboard/SR), and a note to render via `_ux-ui-shared/frameworks/adapter-protocol.md`.

## Accuracy — verify every state, don't assume (mandatory when code is produced)
A component is only "correct" when **every variant × state** renders right — not just the resting default. Build a **states harness**: render the component in each applicable state (default, hover, focus, active, disabled, loading `aria-busy`, error `aria-invalid`, selected `aria-pressed`/`aria-selected`) × each variant in one HTML file (see `examples/component-states/button.html`). Then RUN the gates and report their real output (_ux-ui-shared/CLAUDE.md → Verification Protocol):
- `node _ux-ui-shared/scripts/verify_states.mjs <harness> [--dark]` — contrast of every element in default/hover/focus
- `node _ux-ui-shared/scripts/axe_audit.mjs <harness> [--dark]` — ARIA/role/name/label correctness
- `node _ux-ui-shared/scripts/measure_render.mjs <harness> [--dark]` — every text element AA
- overlays/modals also: `node _ux-ui-shared/scripts/verify_focustrap.mjs <harness> --open=<trigger>`
...
Build mobile-first; a fixed-px width that can't shrink is a bug. Run `node _ux-ui-shared/scripts/verify_responsive.mjs <file|dir>` — it loads each harness at 280/320/414px and fails on any horizontal overflow. The four recurring causes and their fixes:
