...
## Steps
1. Read `_ux-ui-shared/workflows/governance.md` (SemVer table, contribution workflow, deprecation policy, change comms).
...
3. For a **new** component/token: confirm it serves a real, repeated need (≥ 2 places) before promoting product → candidate → core. Design it to the full quality bar (_ux-ui-shared/CLAUDE.md Component Quality Bar).
4. For a **deprecation**: mark with reason + replacement + removal version; keep working ≥ 1 minor cycle; provide a migration map (`_ux-ui-shared/design-systems/crosswalk.md` style); remove only in a major.
5. Wire any new file into `_ux-ui-shared/CLAUDE.md` (File Reference Map + relevant table/router) and add a changelog entry.
...
- `python3 _ux-ui-shared/scripts/validate_tokens.py` passes; contrast re-checked if colors changed.
