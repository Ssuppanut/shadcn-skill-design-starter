...
## Steps
1. Read `_ux-ui-shared/accessibility/wcag-checklist.md` (POUR-organized, P0/P1/P2) and `_ux-ui-shared/accessibility/aria-patterns.md`.
...
4. **Contrast — measure, don't eyeball.** For rendered HTML, RUN the real-render gates and report their actual output (_ux-ui-shared/CLAUDE.md → Verification Protocol): `node _ux-ui-shared/scripts/measure_render.mjs <file> [--dark]` (every text element) AND `node _ux-ui-shared/scripts/verify_states.mjs <file> [--dark]` (every interactive element in default/hover/focus — catches hover-state failures). For loose color pairs, `python3 _ux-ui-shared/scripts/contrast.py "<fg>" "<bg>"`. Never state a ratio you did not measure.
5. Check reduced-motion handling (`_ux-ui-shared/taste/motion-choreography.md`).
